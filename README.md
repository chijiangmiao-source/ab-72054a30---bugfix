...
4. **Playwright**：真实浏览器验证“失败后保留待重试操作 → 恢复后取回唯一镜号”、
   冲突错误反馈与“以新操作重新提交”，以及两个标签页各自绑定自己的逻辑操作、
   放弃继承项与新建操作互不影响、刷新后分别取回 1、2 号。
...
每个浏览器标签页持有独立的标签页标识（`sessionStorage`），自己的待重试操作保存在
`localStorage` 中按标签页区分的槽位里，刷新页面后仍按自己的槽位恢复；另以一个共享键
作为“公告栏”，仅供新打开、自身尚无操作的页面继承展示。因此：

- 在一个标签页放弃继承项、提交新操作或完成自己的操作，都不会清除、替换其他标签页
  尚未恢复的待重试身份；
- 两个标签页先后遭遇“提交后故障”后各自刷新重试，分别取回最初分配的两个号码（均为
  `replayed=true`），场次列表无重复、无缺口，也不会产生第三个号码。

...
2. **pytest**：20 并发无重复无缺口、并发重复重试共享号码、进程重启后映射与计数恢复、
   注入故障只触发一次、409 冲突、参数校验、双客户端各自恢复原号码（另含对 compose 服务的端到端冒烟）；
3. **Vitest**：前端 API 客户端与页面组件对**真实运行的 API** 联调（重试、冲突、并发），
   含双标签页状态回归：记录两个真实请求中的不同 `client_op_id`，覆盖“首页 503 →
   次页放弃继承项 → 次页再产生 503 → 两页分别刷新重试验证各自取回 1、2 号”；
4. **Playwright**：真实浏览器验证“失败后保留待重试操作 → 恢复后取回唯一镜号”、
   冲突错误反馈与“以新操作重新提交”，以及两个真实标签页（独立 sessionStorage）
   互不清除、各自恢复的完整顺序。
...
每个标签页只认自己正在处理的逻辑操作（`web/src/pendingStore.ts`），存储分两层：

- **标签页私有**（sessionStorage）：本页自己的待重试操作，刷新页面后仍在；
  其他标签页的放弃、新建或完成都不会清除或替换它。
- **跨页广播**（localStorage + storage 事件）：最近保存的待重试操作，仅供
  “新打开且没有自己操作”的页面继承一次；已有自己操作的页面一律忽略广播。

因此在同一浏览器打开多个发放页时，各页面可以持有不同的 `client_op_id` 并行
重试、互不占号：每个页面刷新后只会用自己的操作标识取回最初分配的镜号。
